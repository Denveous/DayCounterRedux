...
[assembly: MelonInfo(typeof(DayCounterRedux.DayCounter), "DayCounterRedux", "1.0.13", "Denveous")]
...
namespace DayCounterRedux {
    ...
    public class DayCounter : MelonMod {
        ...
        public override void OnInitializeMelon() {
            ...
            category.CreateEntry("CounterVisible", config.CounterVisible, "Enable Day Counter", "Toggle day counter.");
            category.CreateEntry("CounterLocation", config.CounterLocation, "Counter Location", "1 - topleft, 2 - topright, 3 - bottomleft, 4 - bottomright");
            category.CreateEntry("CounterWidth", config.CounterWidth, "Counter Width", "Width of the day counter UI element.");
            category.CreateEntry("XOffset", config.XOffset, "X Offset", "Horizontal offset for the day counter.");
            category.CreateEntry("YOffset", config.YOffset, "Y Offset", "Vertical offset for the day counter.");
            category.CreateEntry("IconXOffset", config.IconXOffset, "Icon X Offset", "Horizontal offset for the day counter icon.");
            category.CreateEntry("IconYOffset", config.IconYOffset, "Icon Y Offset", "Vertical offset for the day counter icon.");
            category.CreateEntry("UIScale", config.UIScale, "UI Scale", "Scale factor for the day counter.");
            this.LoadConfig();
        }
        ...
        private static Texture2D LoadIconFromResources(string resourceName) {
        ...
        private static void CreateDayIcon() {
            ...
            Texture2D iconTexture = LoadIconFromResources("DayCounterReduxMono.Resources.dayicon.png");
            if (iconTexture != null) {
        ...
        private static void SetupDayTracking() {
            ...
            timeManager.onDayPass += ChangeDayText;
